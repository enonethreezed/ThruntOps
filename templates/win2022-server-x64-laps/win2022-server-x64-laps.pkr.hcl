...
variable "vm_name" {
  type    = string
  default = "win2022-server-x64-laps-template"
}
...
locals {
  template_description = "Windows Server 2022 64-bit LAPS-ready template built ${legacy_isotime("2006-01-02 03:04:05")} — Security/Critical updates applied, Windows LAPS available. username:password => localuser:password"
}

source "proxmox-iso" "win2022-server-x64-laps" {
...
build {
  sources = ["source.proxmox-iso.win2022-server-x64-laps"]
